...
parser = argparse.ArgumentParser(description="Generating query statements and links to literature database searches based on keywords")
parser.add_argument('-m', '--inputMode', metavar='selectionMode', required=True, help='For the first run, enter init, and after that, enter run')
parser.add_argument('-i', '--inputFilePath', metavar='keywordsFile', required=True, help='Enter a text file path that contains only keywords')
parser.add_argument('-o', '--outputFilePath', metavar='outputFile', required=True, help='Output file path')
args = parser.parse_args()

if 'init' == args.inputMode:
    nltk.download('wordnet')
    nltk.download('averaged_perceptron_tagger')
    print('Successfully downloaded wordnet and other thesaurus!')
elif 'run' == args.inputMode:
